...
SOLC_0_4_18="docker run --rm -u `id -u` -v $PWD:/solidity --workdir /solidity/contracts ethereum/solc:0.4.18 --optimize /=/"
...
ABIGEN="docker run --rm -u `id -u` --workdir /go/src/github/tokencard/contracts -e GOPATH=/go -v $GE_PATH:/go/src/github.com/ethereum/go-ethereum -v $PWD:/go/src/github/tokencard/contracts ethereum/client-go:alltools-v1.9.2 abigen"
...
if [ "$1" == "kyber" ] || [ "$1" == "all" ]
    then
        for c in "${kyber_sources[@]}"
        do
            compile_solidity "${SOLC_0_4_18}" $c
        done

        for c in "${kyber_contracts[@]}"
        do
            generate_binding "$c"
        done
fi
